...
echo.
echo => Upgrading pip...
%PYTHON_EXE% -m pip install --upgrade pip --default-timeout=120 -i https://mirrors.aliyun.com/pypi/simple
if errorlevel 1 (
...
%PYTHON_EXE% -m pip install wheel --default-timeout=120 -i https://mirrors.aliyun.com/pypi/simple
if errorlevel 1 (
...
%PYTHON_EXE% -m pip wheel --wheel-dir "%WHEEL_DIR%" --no-deps -i https://mirrors.aliyun.com/pypi/simple mcp[cli]>=1.21.0 httpx>=0.27.0
if errorlevel 1 (
...
rem Download transitive dependencies
%PYTHON_EXE% -m pip wheel --wheel-dir "%WHEEL_DIR%" -i https://mirrors.aliyun.com/pypi/simple -r "%TEMP_REQ%"
set "WHEEL_ERR=!errorlevel!"
del "%TEMP_REQ%" >nul 2>&1

if !WHEEL_ERR! neq 0 (
    echo WARNING: Some transitive dependencies may not have been downloaded
)
